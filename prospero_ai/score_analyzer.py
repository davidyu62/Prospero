...
- 리스크와 기회 모두 언급
- 문장당 한 가지 개념만 담기 (명확성)
- **각 지표별 설명은 최소 5~7문장** (현재값→추이→의미→영향→리스크→기회 순)
- **analysis_summary는 최소 6~8문장** (종합적인 시장 평가)
- 구체적 수치와 시장 맥락을 충분히 포함할 것
...
✓ 정량값을 정성 인사이트로 번역하세요
  예) "BTC가 30일간 -15% 하락했고, 7일간 -8% 추가 하락했으므로, 추세적 약세이면서도
       극단적 공포심리가 나타나고 있습니다. 이러한 약세 추세에도 불구하고 공포탐욕지수가
       극도의 공포 수준(9)을 기록한 것은 시장이 과도하게 위축되었을 가능성을 시사합니다.
       역사적으로 이러한 극단적 공포 구간은 반등의 기회가 되었습니다"
✓ 각 지표별로 **최소 5~7문장** 작성하세요
✓ 시장 국면(regime: {regime})을 고려한 해석을 포함하세요
✓ 투자자 입장에서 "이것이 왜 중요한가", "어떻게 대응할 것인가" 설명
✓ 지표 간 상관관계도 언급 (예: 달러 강세 + 금리 인상 → BTC 약세)
