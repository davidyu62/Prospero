...
[점수 해석 주의 - 반드시 숙지하세요]
- 일부 점수는 '역발상(contrarian)' 방식입니다. 점수가 높다 ≠ 지표가 "좋은/상승" 방향이라는 뜻이 아닙니다.
- btc_trend_score: **BTC 가격이 하락할수록 점수가 높아집니다** (저가 매수 기회 관점). 따라서 이 점수로 가격 방향을 판단하지 마세요.
  → BTC 상승/하락 여부는 **반드시 아래 [핵심 지표 계산값]의 'BTC 30일/7일 변화율(%)' 부호**로만 판단하세요. 음수=하락, 양수=상승.
- fear_greed_score: 공포(지수가 낮을수록)일수록 점수가 높아집니다.
- long_short_score, mvrv_score, funding_rate_score 등도 역발상/저평가 관점이라 점수가 높을수록 "조건이 매수에 유리"하다는 의미이지 지표값 자체가 상승했다는 뜻이 아닙니다.
- 모든 추세·방향 서술은 점수가 아니라 제공된 [핵심 지표 계산값]의 실제 수치에 근거하세요.
...
[핵심 지표 계산값 - 모든 추세·방향 서술의 1차 근거로 사용하세요]
{key_metrics_block}
...
- **BTC 추세(상승/하락)는 반드시 [핵심 지표 계산값]의 'BTC 30일/7일 변화율(%)' 부호로 판단** (btc_trend_score로 판단 금지)
...
class ScoreAnalyzer:
    ...
    def analyze(self, date: str, crypto_data_json: str, macro_data_json: str, score_data: Dict = None) -> Dict:
        ...
        # 계산된 핵심 지표값을 LLM에 명시적으로 전달 (점수만으로 방향을 오판하지 않도록)
        key_metrics_block = self._build_key_metrics_block(crypto_current, macro_current)
    ...
    def _build_key_metrics_block(self, crypto_current: Dict, macro_current: Dict) -> str:
        """계산된 핵심 지표값을 LLM 프롬프트용 텍스트 블록으로 구성

        점수(역발상 로직 포함)만으로는 LLM이 가격 방향을 오판할 수 있으므로,
        format_for_analyzer()가 계산한 실제 변화율·수치를 명시적으로 전달한다.
        """
        def g(d, k, default):
            v = d.get(k, default)
            return default if v is None else v

        btc_change30d = g(crypto_current, "btc_change30d", 0)
        btc_change7d = g(crypto_current, "btc_change7d", 0)
        oi_change30d = g(crypto_current, "oi_change30d", 0)
        yield_spread = g(macro_current, "yield_spread", 0.5)

        lines = [
            f"BTC 현재가: ${g(crypto_current, 'btcPrice', 0):,.0f}",
            f"BTC 30일 변화율: {btc_change30d:+.2f}%  ← 부호로 상승(+)/하락(-) 판단",
            f"BTC 7일 변화율: {btc_change7d:+.2f}%  ← 부호로 상승(+)/하락(-) 판단",
            f"공포탐욕지수: {g(crypto_current, 'fear_greed_current', 50):.0f} (30일 평균 {g(crypto_current, 'fear_greed_avg30d', 50):.0f})",
            f"롱숏비율: {g(crypto_current, 'long_short_ratio', 1.0):.2f}",
            f"미결제약정(OI) 30일 변화율: {oi_change30d:+.2f}%",
            f"MVRV: {g(crypto_current, 'mvrv_current', 1.0):.2f} (30일 평균 {g(crypto_current, 'mvrv_avg30d', 1.0):.2f})",
            f"펀딩비: {g(crypto_current, 'funding_rate', 0.0) * 100:+.4f}% (음수=숏 우위=역발상 매수)",
            f"활성주소: {int(g(crypto_current, 'active_addresses_current', 0)):,} (30일 평균 {int(g(crypto_current, 'active_addresses_avg30d', 0)):,})",
            f"기준금리: {g(macro_current, 'interest_rate_current', 3.0):.2f}%",
            f"10년물금리: {g(macro_current, 'treasury10y', 4.0):.2f}%",
            f"달러인덱스(DXY): {g(macro_current, 'dxy_current', 100):.2f} (30일 전 {g(macro_current, 'dxy_30d_ago', 100):.2f})",
            f"CPI: {g(macro_current, 'cpi', 2.5):.2f}%",
            f"VIX: {g(macro_current, 'vix', 20.0):.2f}",
            f"WTI유가: ${g(macro_current, 'oil_price', 70.0):.2f}",
            f"금리차(T10Y2Y): {yield_spread:+.2f}% (음수=장단기 역전=침체 신호)",
            f"기대인플레이션(10Y BE): {g(macro_current, 'break_even_inflation', 2.3):.2f}%",
        ]
        return "\n".join(lines)
